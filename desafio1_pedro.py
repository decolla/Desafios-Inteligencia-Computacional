...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        # uso de 63 colunas, sendo a última para o norm
        self.W = torch.randn(X.shape[1], DIM_MAX - 1, generator=g) / sigma
        self.b = torch.rand(DIM_MAX - 1, generator=g) * 2 * math.pi
    ...
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        ...
        # aplica cosseno padrão, e nas 22 últimas o dobro da freq 
        c = torch.cat([torch.cos(Z[:, :-22]), torch.cos(2.0 * Z[:, -22:])], dim=1)
        # media dos quadrados dos dados compactado por uma tanh()
        norm = 0.8 * torch.tanh((Xs ** 2).mean(dim=1, keepdim=True) - 1.0)
        return torch.cat([math.sqrt(2 / DIM_MAX) * c, norm], dim=1)
